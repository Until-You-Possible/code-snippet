...
if (typeof window.util !== "object" && typeof window.util !== "function") {
    window.util = {};
}
...
(function (string) {
    ...
    if (typeof window.util.Device === "object" || typeof window.util.Device === "function") {
        var apiVersion = "${version}";
        window.util.Device._checkAPIVersion(apiVersion);
        return;
    }
    ...
    const oLogger = new DeviceLogger();
    ...
    function detachEvent(sEventId, fnFunction, oListener) {
        const aEventListeners = mEventRegistry[sEventId];

        if (!aEventListeners) {
            return this;
        }

        let i = 0, iL = aEventListeners.length;
        for (; i < iL; i++) {
    ...
    function fireEvent(sEventId, mParameters) {
        let aEventListeners = mEventRegistry[sEventId];
        let oInfo;
        if (aEventListeners) {
    ...
    function getOS(userAgent, platform) { // may return null!!
        ...
        function getDesktopOS() {
            const sPlatform = platform || navigator.platform;
    ...
    function getBrowser(customUa, customNav) {
        ...
        function calcBrowser(customUa) {
            const sUserAgent = (customUa || ua).toLowerCase(); // use custom user-agent if given

            const rwebkit = /(webkit)[ \/]([\w.]+)/;
            const ropera = /(opera)(?:.*version)?[ \/]([\w.]+)/;
            const rmozilla = /(mozilla)(?:.*? rv:([\w.]+))?/;

            const browserMatch = rwebkit.exec(sUserAgent) ||
        ...
        const oBrowser = calcBrowser(customUa);
        const sUserAgent = customUa || ua;
        const oNavigator = customNav || window.navigator;
    ...
    function matchLegacyBySize(iFrom, iTo, sUnit, iSize) {
        function convertToPx(iValue, sUnit) {
            if (sUnit === "em" || sUnit === "rem") {
                const fnGetStyle = window.getComputedStyle || function (e) {
        ...
        const width = iSize[0];
        const a = iFrom < 0 || iFrom <= width;
        const b = iTo < 0 || width <= iTo;
        return a && b;
    }
    ...
    function getSystem(customUA) {
        const bTabletDetected = !!isTablet(customUA);
    ...
    if (util.define) {
        util.define("/util/Device", [], function() {
            return Device;
        });
    }
